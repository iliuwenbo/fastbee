...
import static com.fastbee.common.core.redis.RedisKeyBuilder.buildModbusTcpCacheKey;
import static com.fastbee.common.core.redis.RedisKeyBuilder.buildModbusTcpRuntimeCacheKey;
...
public class RedisCache {
    ...
    public Map<String, String> hashGetAllMatchByKeys(Set<String> keys, String operator, String id, String triggerValue, String modelIndex) {
        ...
        if (id.startsWith("array_")) {
            ...
            List<String> list = StringUtils.str2List(id, "_", true, true);
            ...
            while (it.hasNext()) {
                ...
                for (Map.Entry entry : result.entrySet()) {
                    ...
                    if (field.equals(matchId) || field.equals(matchId + "#V")) {
                        ...
                        // 数组或数组对象元素索引
                        if (indexValue >= 0) {
                            List<String> list = StringUtils.str2List(value, ",", true, true);
     ...
     * @return 缓存键值对应的数据
     */
    public String getStrCacheObject(final String key) {
     ...
     * @param key
     */
    public void deleteStrObject(final String key) {
     ...
     * @param key
     */
    public void deleteStrHash(final String key) {
    ...
    public <T> Object getStringHashValue(final String key, final String hKey) {
        return stringRedisTemplate.opsForHash().get(key, hKey);
    }

    public <T> void delStringHashValue(final String key, final String hKey) {
    ...
    public synchronized String getCacheModbusTcpId(String serialNumber) {
        ...
        String key = buildModbusTcpCacheKey(serialNumber);
        String oldId = stringRedisTemplate.opsForValue().get(key);
        long id = 0;
        if (StringUtils.isBlank(oldId)) {
            stringRedisTemplate.opsForValue().set(key, String.valueOf(0));
        } else {
            int oldI = Integer.parseInt(oldId);
            if (oldI < 65535) {
    ...
    public synchronized void cacheModbusTcpData(String serialNumber, String id, String data) {
        String dataKey = buildModbusTcpRuntimeCacheKey(serialNumber);
        stringRedisTemplate.opsForHash().put(dataKey, id, data);
        stringRedisTemplate.expire(dataKey, 30, TimeUnit.SECONDS);
    }
