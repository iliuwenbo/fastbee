...
public class SceneServiceImpl implements ISceneService {
    ...
    public int updateScene(Scene scene) {
        ...
        // 批量删除规则脚本
        scriptService.deleteRuleScriptBySceneIds(new Long[]{scene.getSceneId()});
        // 批量删除场景脚本
        sceneScriptService.deleteSceneScriptBySceneIds(new Long[]{scene.getSceneId()});
        // 批量删除场景设备
    ...
    public int deleteSceneBySceneIds(Long[] sceneIds) {
        for (Long sceneId : sceneIds) {
            ...
            List<SceneMiddle> sceneMiddles = sceneMiddleService.list(lqw);
            if (!sceneMiddles.isEmpty()) {
                throw new BaseException("存在未删除的场景构建！");
            }
    ...
    public int updateStatus(Scene scene) {
        ...
        // 小包设置关闭状态
        updateScene.setEnable(2);
        //更新中报
        sceneMiddleService.updateBySceneId(scene.getSceneId());
...
//        LiteFlowChainELBuilder.createChain().setChainName(scene.getChainName()).setEL(scene.getElData()).build();
    ...
    private String buildTriggerAction(SceneScript sceneScript, int scriptPurpose, Scene scene, List<Script> ruleScripts, List<SceneScript> sceneScripts, List<SceneDevice> sceneDevices) {
        ...
        ScriptTemplate template = new ScriptTemplate(
                scene.getSceneId(), scene.getRecoverId(), scriptId, sceneScript.getType(), sceneScript.getSource(), scene.getSilentPeriod(),
                scene.getExecuteDelay(), scene.getCheckDelay(), scene.getCond(), sceneScript.getScriptPurpose(), sceneScript.getId(), sceneScript.getValue(),
    ...
    public void openSceneLog(String chainName) {
        flowExecutor.openSceneLog(chainName);
    }

    @Override
    public void closeSceneLog(String chainName) {
        flowExecutor.closeSceneLog(chainName);
    }
